...
// `jwkfetch.WithWaitReady(true)`, and how to tell "the JWKS endpoint has not
// answered yet" apart from other registration failures.
//
// `(jwkfetch.Cache).Register` with `jwkfetch.WithWaitReady(true)` (the
// default) blocks until the first fetch of the JWKS succeeds. That wait has no
// timeout of its own, because the cache keeps retrying the URL on its refresh
// schedule and only the caller knows how long a JWKS outage at startup is
// worth waiting for. Pass a `context.Context` carrying a deadline to bound it.
//
...
func Example_jwk_cache_waitready() {
	...
	err = c.Register(regCtx, srv.URL, jwkfetch.WithWaitReady(true))
